...
describe Atm do
  ...
  it 'rejects withrawal if account is disabled' do
    ...
    expect(subject.withdraw(6, account.pin_code, account)).to eq expected_output
  end
  ...
  it 'rejects withdrawal if account has insufficient funds' do
    ...
    expect(subject.withdraw(155, account.pin_code, account)).to eq expected_output
  end
  ...
  it 'rejects withdrawal if ATM has insufficient funds' do
    ...
    expect(subject.withdraw(100, account.pin_code, account)).to eq expected_output
  end
end
