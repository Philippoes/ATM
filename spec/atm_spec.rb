...
describe Atm do
  let(:account) { instance_double('Account', pin_code: rand(1000..9999), exp_date: Date.today.next_year(5).strftime('%m/%y'), account_status: :Active) }
  ...
  it 'should be able to withdraw' do
    subject.withdraw(50, account.pin_code, account.exp_date, :Active, account)
  ...
  it 'rejects withrawal if account is disabled' do
    ...
    expect(subject.withdraw(6, account.pin_code, account.exp_date, :Disabled, account)).to eq expected_output
  end

  it 'rejects withdrawal if pin is wrong' do
    expected_output = { status: false, message: 'wrong pin', date: Date.today}
    expect(subject.withdraw(50, 9999, account.exp_date, :Active, account)).to eq expected_output
  end
  ...
  it 'rejects withdrawal if account has insufficient funds' do
    ...
    expect(subject.withdraw(155, account.pin_code, account.exp_date, :Active, account)).to eq expected_output
  end
  ...
  it 'will allow withdrawal if account has enough funds' do
    ...
    expect(subject.withdraw(50, account.pin_code, account.exp_date, :Active, account)).to eq expected_output
  end

  it 'rejects withdrawal if ATM has insufficient funds' do
    subject.funds = 50
    expected_output = { status: false, message: 'insufficient funds in ATM', date: Date.today}
    expect(subject.withdraw(100, account.pin_code, account.exp_date, :Active, account)).to eq expected_output
  end
end
